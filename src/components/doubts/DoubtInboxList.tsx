  ...
  return (
    <Card style={{ padding: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
        <div>
          <div style={{ fontWeight: 700, fontSize: 14.5 }}>{doubt.studentName}</div>
      ...
      <div style={{ marginTop: 10, fontSize: 13.5 }}>{doubt.body}</div>

      {doubt.answeredByName ? (
        <div style={{ marginTop: 8, fontSize: 12.5, color: "var(--muted)" }}>Last answered by {doubt.answeredByName}</div>
      ) : null}
...
export function DoubtInboxList({ doubts }: { doubts: DoubtRow[] }) {
  if (doubts.length === 0) {
    return <Card style={{ padding: 20 }}>No questions yet.</Card>;
  }
